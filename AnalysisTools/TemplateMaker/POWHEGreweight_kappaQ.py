...
if os.path.isfile("/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst_kappaQ/{}/{}/templateOutput_{}_{}_final2.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])): sys.exit()
...
for i, fname in enumerate(tqdm(treelist)):
    ...
    GammaH = 0.004088
    # naming convention
    # pure_SIG_t=0 tB=1 BKG=2 pureQ=3 QB=4 tQ=5
   
    for iev,event in tqdm(enumerate(t)):
        if iev==0: weight_nom = k3nlo * lumi * 1000 * event.genxsec * event.genBR / event.Bin40
        ...
        mass_rw = ( (event.GenHMass**2 - (125**2))**2 + ((125**2)*GammaH**2) )/( (event.GenHMass**2 - 125.38**2)**2 + ((125.38**2)*GammaH**2 ) )
        #print ("factor: ", event.GenHMass ,mass_rw)
        ...
        if sys.argv[2] == '0':
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, mass_rw * wsyst * (event.p_Gen_GG_SIG_kappaTopBot_1_ghz1_1_MCFM) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
        elif sys.argv[2] == '1':
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, sqrt(mass_rw) * wsyst * (event.p_Gen_GG_BSI_kappaTopBot_1_ghz1_1_MCFM-event.p_Gen_GG_SIG_kappaTopBot_1_ghz1_1_MCFM-event.p_Gen_GG_BKG_MCFM) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
        elif sys.argv[2] == '2':
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, wsyst * (event.p_Gen_GG_BKG_MCFM) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            ...
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, mass_rw * wsyst * (event.p_Gen_GG_SIG_kappa4genTop_1_ghz1_1_MCFM/event.Native) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
            
        elif sys.argv[2] == '4':
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, sqrt(mass_rw) * wsyst * (event.p_Gen_GG_BSI_kappa4genTop_1_ghz1_1_MCFM/event.Native - event.p_Gen_GG_SIG_kappa4genTop_1_ghz1_1_MCFM/event.Native - event.p_Gen_GG_BKG_MCFM) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
            
        elif sys.argv[2] == '5':
            if event.Native==0: continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, mass_rw * wsyst * (event.p_Gen_GG_SIG_kappaTop_1_kappa4genTop_1_ghz1_1_MCFM/event.Native - event.p_Gen_GG_SIG_kappaTop_1_ghz1_1_MCFM/event.Native - event.p_Gen_GG_SIG_kappa4genTop_1_ghz1_1_MCFM/event.Native) * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst_kappaQ/{}/{}/templateOutput_{}_{}_parts2.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst_kappaQ/{}/{}/templateOutput_{}_{}_final2.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])
