...
if os.path.isfile("/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst/{}/{}/templateOutput_{}_{}_final.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])): sys.exit()
...
for i, fname in enumerate(tqdm(treelist)):
    ...
    for iev,event in tqdm(enumerate(t)):
        ...
        if sys.argv[2] == 'SIG':
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, wsyst * event.p_Gen_GG_SIG_kappaTopBot_1_ghz1_1_MCFM * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
        elif sys.argv[2] == 'BSI':
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, wsyst * event.p_Gen_GG_BSI_kappaTopBot_1_ghz1_1_MCFM * event.KFactor_QCD_ggZZ_Nominal * event.p_Gen_CPStoBWPropRewgt * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
        elif sys.argv[2] == 'BKG':
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst/{}/{}/templateOutput_{}_{}_parts.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/POWHEGtemplates/nominal_w_syst/{}/{}/templateOutput_{}_{}_final.root".format(tagpath[sys.argv[3]], sys.argv[1], sys.argv[2], sys.argv[4])
