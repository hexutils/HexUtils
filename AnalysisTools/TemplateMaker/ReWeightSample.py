...
def Reweight_Event(InputEvent,ProductionMode,InputHypothesis,OutputHypothesis,HffHypothesis,isData):
  ...
  namespace = {'weight': 1.,'InputEvent':InputEvent,'MelaConstantNames':MelaConstantNames}
  for i in range(len(MelaConstantNames)):
    exec("weight = weight * InputEvent."+MelaConstantNames[i],namespace)
