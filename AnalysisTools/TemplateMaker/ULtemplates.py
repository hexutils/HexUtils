...
treelist2016APV = glob.glob('/eos/user/g/gritsan/Write/TaggedTrees/UL_CJLST_test/cjlst/Run2UL_22/Hadded/MC_offshell/MC2016APV/VBF*/ZZ4lAnalysis.root')
treelist2016 = glob.glob('/eos/user/g/gritsan/Write/TaggedTrees/UL_CJLST_test/cjlst/Run2UL_22/Hadded/MC_offshell/MC2016/VBF*/ZZ4lAnalysis.root')
treelist2017 = glob.glob('/eos/user/g/gritsan/Write/TaggedTrees/UL_CJLST_test/cjlst/Run2UL_22/Hadded/MC_offshell/MC2017/VBF*/ZZ4lAnalysis.root')
treelist2018 = glob.glob('/eos/user/g/gritsan/Write/TaggedTrees/UL_CJLST_test/cjlst/Run2UL_22/Hadded/MC_offshell/MC2018/VBF*/ZZ4lAnalysis.root')
...
for i, fname in enumerate(tqdm(treelist)):
    ...
    if sys.argv[2] == 'SIG':
        ...
        for iev,event in tqdm(enumerate(t)):
            if iev==0: weight_nom = lumi * event.xsec / event.Bin40
            if not (event.ZZMass >= 220): continue
            if event.EventTag==-999: continue
            ZZFlav = event.Z1Flav*event.Z2Flav
            if not ( (ZZFlav)==(121*121) or (ZZFlav)==(169*169) or (ZZFlav)==(121*169) ): continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, event.p_Gen_JJEW_SIG_ghv1_1_MCFM * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
        ...
        for iev,event in tqdm(enumerate(t)):
            ...
            ZZFlav = event.Z1Flav*event.Z2Flav
            if not ( (ZZFlav)==(121*121) or (ZZFlav)==(169*169) or (ZZFlav)==(121*169) ): continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, event.p_Gen_JJEW_BSI_ghv1_1_MCFM * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
        ...
        for iev,event in tqdm(enumerate(t)):
            ...
            ZZFlav = event.Z1Flav*event.Z2Flav
            if not ( (ZZFlav)==(121*121) or (ZZFlav)==(169*169) or (ZZFlav)==(121*169) ): continue
            hist.Fill(event.ZZMass, event.Dbkg, event.Dbsi, event.p_Gen_JJEW_BKG_MCFM * event.overallEventWeight * event.L1prefiringWeight * weight_nom)
            histref.Fill(event.ZZMass, event.Dbkg, event.Dbsi, 1)
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/ULtemplates/VBF/inclusive/{}/templateOutput_{}_{}_parts.root".format(sys.argv[1], sys.argv[1], sys.argv[2])
...
histout = ROOT.TH3F('hist_{}'.format(sys.argv[2]), 'UL VBF {} {}'.format(sys.argv[1], sys.argv[2]), len(medges)-1, medges, len(d1edges)-1, d1edges, len(d2edges)-1, d2edges)
...
foutname = "/eos/user/l/lkang/www/research/Sandbox/ULtemplates/VBF/inclusive/{}/templateOutput_{}_{}_final.root".format(sys.argv[1], sys.argv[1], sys.argv[2])
