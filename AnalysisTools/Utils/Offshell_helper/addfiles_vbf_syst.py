...
treelistpath = "./vbf_output_temps.txt"
...
for el in paths: 
    ...
    if procsyst == "" : 
        fout = ROOT.TFile("./Templates/vbf_"+el[0]+"_"+el[1]+".root","recreate")
    else : 
        fout = ROOT.TFile("./Templates_syst/vbf_"+el[0]+"_"+el[1]+"_"+procsyst+"_temporary.root","recreate")
...
if procsyst == "": 
    os.system("hadd -f -k  vbf_Untagged.root  ./Templates/*Untagged*.root")
    os.system("hadd -f -k vbf_VHtagged.root  ./Templates/*VHtagged*.root")
    os.system("hadd -f -k vbf_VBFtagged.root  ./Templates/*VBFtagged*.root")
else : 
    os.system("hadd -f -k vbf_Untagged_"+procsyst+".root  ./Templates_syst/*Untagged*"+procsyst+"_temporary.root")
    os.system("hadd -f -k vbf_VHtagged_"+procsyst+".root  ./Templates_syst/*VHtagged*"+procsyst+"_temporary.root")
    os.system("hadd -f -k vbf_VBFtagged_"+procsyst+".root  ./Templates_syst/*VBFtagged*"+procsyst+"_temporary.root")
...
if not procsyst == "" : 
    ...
    for cat in category: 
        fnom = ROOT.TFile("vbf_"+cat+".root")
        fsyst = ROOT.TFile("vbf_"+cat+"_"+procsyst+".root")
    ...
    if fix :          
        print ("fixing...")
        for cat in category: 
            fsyst_fix = ROOT.TFile("vbf_"+cat+"_"+procsyst+"_fixed.root","recreate")
            fsyst = ROOT.TFile("vbf_"+cat+"_"+procsyst+".root")
            lhists = []
