...
def main(argv):
    ...
    if not os.path.exists(filename):
        ...
        for tind, tree in enumerate(treenames):
            ...
            if tree == "candTree": eventrange = 11658
            else: eventrange = t.GetEntries()

            for ent in trange(eventrange):
            ...
            if tree == "candTree": exec("new{} = t.CloneTree(11658, 'fast')".format(tree))
            else: exec("new{} = t.CloneTree(-1, 'fast')".format(tree))
